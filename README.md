...
Neural systems produce continuous representations that capture structure no human formalism was designed to express. These representations are information-rich and completely opaque.

LFM makes them speakable. It encodes any continuous representation as a new natural language — not English, not mathematics, but an emergent language whose morphology and phonology are inherited from a pretrained multilingual decoder. The language is alien but *structurally natural*: compositional, variable-length, phonotactically regular. A pretrained LLM can learn to translate it the same way it would learn any unfamiliar human language — and because the language itself encodes empirical grounding, learning the language means acquiring that grounding.

The mechanism is a **frozen linguistic bottleneck**: a VAE decoder pretrained on 16 typologically diverse languages, then frozen. Downstream systems project their representations into the decoder's latent space; the decoder constrains the output to be linguistically well-formed. This is analogous to Universal Grammar — a fixed structural prior where only the mapping from meaning to form is learned. It avoids the failure modes of end-to-end emergent communication (degenerate codes, non-compositional signals, anti-Zipfian distributions).

The pipeline is concrete: embedding → VAE latent space → frozen decoder → IPA tokens. A receiver can identify what was encoded (95% accuracy, 15x above chance). An LLM can translate the emergent IPA into English. Every step is empirically grounded and measurable.
