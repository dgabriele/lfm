...
In linguistic terms, LFM is a **general neural onomasiology** — a concept-to-form mapping. Where classical onomasiology tried to lexicalize hand-defined conceptual primitives, LFM lexicalizes whatever continuous representation a source neural system happens to produce: no metaphysics required, the "concept" is simply the activation pattern. The frozen decoder *is* the **lexicalization function**: its pretrained phonotactic and morphological priors constrain every source state to surface as a compositional utterance, with related source states producing related utterances (topology preservation). Productive subword composition — the system generating novel words from morphemes the way natural languages coin new terms — emerges as a consequence of those priors, not as a bolted-on capability.
...
- **[Corpus Analysis](docs/corpus-analysis.md)** — Distributional and grammatical analysis of the Neuroglot corpus: word categories, syntactic transitions, Zipf's law, productivity, compositionality. 10 diagnostic visualizations.
