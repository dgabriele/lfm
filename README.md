...
LFM makes them speakable. It encodes any continuous representation as a new natural language — not English, not mathematics, but an emergent language with its own morphology and phonology, whose structure is inherited from a pretrained multilingual decoder. The language is alien but *structurally natural*: it shares the inductive biases of human languages (compositionality, variable-length encoding, phonotactic regularity), which means a pretrained multilingual LLM can learn to translate it the same way it would learn any unfamiliar natural language. More than that — because the LLM learns from corpora grounded in the same empirical space as the language itself, the language itself encodes empirical grounding. An LLM trained on LFM-encoded protein descriptions isn't just translating symbols — the language carries the grounding, and learning the language means acquiring it.
