...
**The expression system** uses a DiffusionZGenerator (flow-matching denoiser, 4 layers, T=4 steps) that produces multi-segment z-sequences, each decoded through the frozen decoder. Per-position activity scores enable variable-length output. An IPA-to-IPA receiver with soft topology loss (KL divergence against input embedding cosine similarities) ensures the expressions preserve semantic structure. The expressions are romanized to natural-looking ASCII orthography for LLM consumption.
...
Each major component — the multilingual decoder, the diffusion z-sequence generator, the IPA-to-IPA receiver, the self-supervised translation pipeline, the visualization and evaluation suite — has been developed and validated independently. The current focus is end-to-end integration: generating a learnable corpus, training an LLM to understand it, and testing whether translation reveals the source system's native perspective on its domain.
