...
def run_epoch_diagnostics(
    *,
    ...
    for j in range(min(2, len(_lang_labels))):
        ...
        wer = ed / max(ml, 1)
        logger.info(
            "  recon[%s] orig: %s", _lang_labels[j], orig
        )
        logger.info(
            "  recon[%s]  dec: %s  [WED=%d/%d WER=%.0f%%]",
            _lang_labels[j], dec, ed, ml, wer * 100,
        )
    ...
    for k, (s, txt) in enumerate(zip(noise_scales, perturb_texts)):
        logger.info("  perturb[σ=%.1f]: %s", s, txt)
    ...
    for j in range(3):
        logger.info("  random[%d]: %s", j, random_texts[j])
    for j in range(3, 5):
        logger.info(
            "  near_eng[%d]: %s", j - 3, random_texts[j]
        )
