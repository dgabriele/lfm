"""Shared base class for phrase-VAE generators.

Factors out the VAE architecture (encoder, reparameterization, frozen
decoder, latent-space calibration, KL/reconstruction objective) from
the alphabet-specific bits (tokenizer, ``decode_to_text``).  Concrete
subclasses plug in a particular token alphabet by implementing
:meth:`_init_tokenizer` and :meth:`decode_to_text`.

Two sibling subclasses live alongside this base:

* :class:`~lfm.generator.multilingual_vae.MultilingualVAEGenerator` —
  the original SPM-over-IPA design, retained as a usable option.
* :class:`~lfm.generator.phoneme_vae.PhonemeVAEGenerator` (forthcoming)
  — a phoneme-level ASCII alphabet chosen for stable tokenization by
  downstream LLMs (Qwen's BPE in particular), so generated Neuroglot
  surface forms survive the round-trip through an interpreter model.

Both subclasses share everything in this base: encoder, z-projection,
frozen decoder, KL loss, calibration, pretrained-weight loading.
"""
...
class BaseVAEGenerator(GeneratorModule):
    ...
    Subclasses provide only the tokenizer and the ``decode_to_text``
    implementation.  Everything else — the encoder, latent sampling,
    autoregressive decode, KL regularization, latent calibration,
    pretrained-weight loading — is inherited unchanged.

    Args:
        config: Generator configuration.  The subclass is responsible
            for interpreting ``vocab_size`` and any alphabet-specific
            config fields consistently with the tokenizer it installs.
    """
    ...
    def __init__(self, config: GeneratorConfig) -> None:
        ...
        # -- Tokenizer (delegated to subclass) --
        # Subclass sets self._tokenizer; base class does not assume a type.
    ...
    # ------------------------------------------------------------------
    # Subclass-plugin points
    # ------------------------------------------------------------------

    def _init_tokenizer(self, config: GeneratorConfig) -> None:
        """Install a tokenizer on ``self._tokenizer``.

        Called exactly once during ``__init__``.  The base class default
        is a no-op; subclasses should override to wire up their alphabet-
        specific tokenizer (e.g. SPM-over-IPA, phoneme-level ASCII).
        """
        self._tokenizer = None

    @abstractmethod
    def decode_to_text(self, token_ids: Tensor) -> list[str]:
        """Decode integer token ids to strings in the subclass's alphabet.

        This is the only output-facing method that must vary between
        VAE variants; everything else in the architecture is shared.
        """
        ...
