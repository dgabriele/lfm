...
export function ConfigsSidebar({ configs }: { configs: VaeModelRow[] }) {
  return (
    <aside className="w-80 shrink-0 border-l border-separator bg-surface/40 p-6 flex flex-col gap-4 overflow-y-auto">
      <header>
      ...
      {configs.length === 0 ? (
        ...
        <ul className="flex flex-col gap-1.5">
          {configs.map((c) => (
            <li
              key={c.id}
              className="flex items-center gap-2 rounded-[calc(var(--radius)*0.6)] px-2 py-1.5 hover:bg-default/40 transition-colors"
            >
